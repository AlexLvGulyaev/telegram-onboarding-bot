...
<img src="docs/screenshots/TOB_portfolio_light.png" alt="Telegram Onboarding Bot — витрина кейса: чат обучения в Telegram, тест и результат в PostgreSQL (LIGHT)">
...
- HR добавляет тему «Информационная безопасность» через `/new_topic` (затем активирует её через `/set_topic`) — и следующий сотрудник сразу проходит по ней обучение, без правки кода.
...
├── docker-compose.yml                   # Инфраструктура: bot + db + redis
...
MVP с универсальной архитектурой тем и промптов. E2E-дефекты устранены и подтверждены прогонами (см. [`docs/TESTING.md`](docs/TESTING.md)). Deployment Validation пройдена в чистом окружении со статусом PASS (см. [`docs/DEPLOYMENT_VALIDATION_REPORT.md`](docs/DEPLOYMENT_VALIDATION_REPORT.md)).
...
- **Сессии в памяти без REDIS_URL** — если `REDIS_URL` не задан (dev-режим), прогресс активной сессии теряется при перезапуске бота (`MemoryStorage`). В prod-развёртывании FSM-сессии хранятся в Redis; завершённые результаты — в PostgreSQL.
...
**Статус:** Кейс закрыт — Deployment Validation PASS (30.08), прод-редеплой верифицирован живым прогоном
