...
Команда `/topic` без аргументов показывает список доступных тем и тему,
обучением по которой вы будете заниматься после `/start`.

> ℹ️ Смена темы — операционное действие: тема общая для всей команды, поэтому
> `/topic <id>` доступна только администратору (см.
> [🎛️ `docs/OPERATOR_GUIDE.md`](OPERATOR_GUIDE.md)). Сотрудник начинает
> обучение по текущей активной теме командой `/start`.

Справку по всем командам бот выдаёт по `/help`.
